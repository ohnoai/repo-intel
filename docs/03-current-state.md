...
| NEW-GIT-BASE-MESSAGE | ⚪ Low | open | The "diff base resolved to HEAD (no local main and no upstream branch)" message fires whenever the base is the literal `HEAD`, including an explicit `--base HEAD` (`collect-git.mjs`, the `rawBaseRef === "HEAD"` check after the C4 precedence block). On a repository that does have a local `main` and an upstream, that wording is false. Found 2026-09-20 while reviewing S3 step 3. The wording predates S3 and step 3 keeps its text exactly, so it was left alone. S3 moves it from a warning to an observation (`diff-base-resolved-to-head`), so it no longer demotes status; what remains is misleading wording. The evidence-label effect (the `diff` subtree marked unresolved) is correct in both cases and should not change. Suggested fix: emit the current text only on the fallback path and give the explicit case its own wording. Confirmed by reading the code and by a scratch-repo run during the S3 step 3 review; no test covers it. |
