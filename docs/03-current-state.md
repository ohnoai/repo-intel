...
| Lows | ⚪ Low | confirmed | doc-discoverability, npm-script gap, product `metadata.product` hardcode, YAML tab / quoted-`#` heuristics, determinism test never git-inits, `compareText` duplicated verbatim across eight files (`lib/observations.mjs` and seven pre-existing copies; flagged by the S3 ultra review, a shared `lib/compare.mjs` is future cleanup, not part of S3). Untouched. |
