...
import { classifyGitWorkTreeProbe } from "./git-worktree-probe.mjs";
...
function attachGitEvidenceLabels(metadata, { baseResolvedToHead = false } = {}) {
  ...
  // A null baseRef (rejected, or never attempted because includeDiff was false), a baseRef
  // that resolved to HEAD, and an omitted diff (never attempted) are all cases where nothing
  // under `diff` asserts anything meaningful about the repository (§6.2's `diff.baseRef` row;
  // §6.2's `diff` when `omitted: true` row; example §7.2). Rather than overriding
  // baseRef/changedFiles/numstat individually, blanket the whole subtree with one nested
  // `unresolved` default, per §3.3 -- except `omitted` itself, which is a direct read of the
  // collector's own control flow and stays `observed_fact` regardless of what the rest of the
  // subtree says.
  const diffUnresolved = metadata.diff.baseRef === null || baseResolvedToHead || metadata.diff.omitted;
...
function initialMetadata() {
  return {
    ...
    diff: {
      ...
      changedFiles: [],
      numstat: [],
      // True until the collector actually attempts a diff (flipped right before it does,
      // regardless of whether that attempt then succeeds, is rejected, or finds no base).
      // Replaces export.mjs's old withoutDiff(), which deleted this key outright instead of
      // stubbing it -- see docs/decisions/2026-09-20-warnings-vs-observations.md's successor
      // S6 design record and RI-07(b).
      omitted: true,
    },
  };
}
...
export function collectGitInventory(
  ...
  const probe = classifyGitWorkTreeProbe(insideWorkTree, repositoryRoot);
  if (probe !== "inside") {
    // "unknown" is a failure Git didn't explain (lock, timeout, permission error on a
    // directory that looks like a repository). Reporting it as "not a repository" would
    // misstate a real repo, so it stays null (unresolved) like the Git-missing case.
    const unresolved = probe === "git-missing" || probe === "unknown";
    metadata.repository.isGitRepository = unresolved ? null : false;
    metadata.repository.isRepositoryRoot = unresolved ? null : false;
    ...
    return {
      ...
      warnings: [
        probe === "git-missing"
          ...
          : probe === "unknown"
            ? "Git could not determine whether this directory is a repository (the command failed or timed out); Git inventory was not collected."
  ...
  if (baseRejected) {
    ...
    if (baseResolvedToHead) {
      // S3: this is a fact about the repository (no local main, no upstream), not a
      // collection failure -- the message says so itself -- so it must not go through
      // warn() and demote status. See docs/decisions/2026-09-20-warnings-vs-observations.md
      // section 2.4 and section 5 step 3.
      observations.push({
