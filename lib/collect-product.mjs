...
// §6.6, with the §10.4 V1-2 simplification: product identity collapses from three cases to
// two. A configured name whose signals never fired and no product config at all are both
// "unresolved" -- neither is a claim this collector can stand behind -- rather than treating
// the former as an inferred negative.
function productLabel(product) { return product !== null ? "documented_intent" : "unresolved"; }
...
function sourcePaths(root) {
  ...
  function visit(directory, depth = 0) {
    ...
    for (const entry of entries.sort((a, b) => compareText(a.name, b.name))) {
      ...
      // S7: entry.name alone (the bare filename), not `absolute` -- isPrivateEnvironmentFile
      // only ever looks at the last path segment, and `absolute` is OS-path-separated
      // (backslashes on Windows), which split("/") would not parse correctly.
      if (
        entry.isFile() &&
        SOURCE_EXTENSIONS.test(entry.name) &&
        !isPrivateEnvironmentFile(entry.name)
      ) {
...
export function collectProduct({ root = process.cwd() } = {}) {
  ...
  try { if (!statSync(repositoryRoot).isDirectory()) throw new Error("not a directory"); }
    ...
    return {
      ...
      observations: [],
      metadata: { product: null, signals: [], riskSurfaces: [], evidenceLabels: { default: "unresolved", fields: {} } },
    };
  }
  ...
  const signals = [...signalPaths].filter(([, paths]) => paths.size).map(([id, paths]) => ({
    id,
    evidencePaths: [...paths].sort(compareText),
    evidenceLabels: { default: "documented_intent", fields: sortedFields({ evidencePaths: "mechanical_inference" }) },
  })).sort((a, b) => compareText(a.id, b.id));
  const riskSurfaces = RISKS.map(([id, severity]) => ({
    id,
    severity,
    detected: riskPaths.get(id).size > 0,
    evidencePaths: [...riskPaths.get(id)].sort(compareText),
    evidenceLabels: { default: "documented_intent", fields: sortedFields({ detected: "mechanical_inference", evidencePaths: "mechanical_inference" }) },
  ...
  return {
    ...
    records: [{
      ...
      product,
      inspectedSourceFiles: inspected,
      evidenceLabels: { default: "observed_fact", fields: sortedFields({ product: label }) },
    }],
    ...
    metadata: {
      ...
      signals,
      riskSurfaces,
      evidenceLabels: { default: "documented_intent", fields: sortedFields(label === "unresolved" ? { product: "unresolved" } : {}) },
    },
  };
}
