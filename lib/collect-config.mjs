...
import { buildObservations } from "./observations.mjs";
import {
...
function scanEnvironmentReferences(root, localPath, environmentVariables, warnings, observations) {
  ...
  for (const match of text.matchAll(/\b(process\.env|import\.meta\.env)\b/g)) {
    ...
    if (directAccess) {
      ...
      observations.push({
        ...
        message: `An environment variable access could not be resolved statically: ${path}:${line}.`,
      });
    }
  }
}
...
function shouldSkipEnvironmentScan(localPath) {
  ...
  // S7: verified isExcludedRepositoryPath (below) already calls isEnvironmentFile, so a
  // ".env.ts"-shaped source file is already excluded here without any change -- unlike the
  // product and delivery collectors, this walker had no gap. No isPrivateEnvironmentFile
  // check needed; kept as-is to avoid a redundant, untestable no-op.
  return (
...
function scanSourceDirectory(root, localDirectory, environmentVariables, warnings, observations) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      scanSourceDirectory(root, localPath, environmentVariables, warnings, observations);
    } else if (entry.isFile() && SOURCE_EXTENSIONS.has(extname(entry.name).toLowerCase())) {
      scanEnvironmentReferences(root, localPath, environmentVariables, warnings, observations);
    }
  }
}
...
function environmentRecords(environmentVariables, observations) {
  const records = [];
  ...
  for (const record of environmentVariables.values()) {
    ...
    if (
      ...
      observations.push({
        ...
        message: `Client-prefixed environment variable evidence also appears in server code: ${record.name}.`,
      });
    }
...
function collectRootEnvironmentReferences(root, environmentVariables, warnings, observations) {
  let entries;
  ...
  for (const entry of entries.sort((left, right) => compareText(left.name, right.name))) {
    ...
    scanEnvironmentReferences(root, entry.name, environmentVariables, warnings, observations);
  }
}
...
// 6.3 to a single configuration record. `type`/`format` are always mechanical_inference -
// assigned from the filename/extension, never declared by the file itself. A malformed
// record short-circuits everything else: nothing was actually extracted, so every
// parsed-content field (there may be none, as with `malformedRecord()`, or some, as with a
// malformed TOML overview that still carries partial `settings`/`environmentVariables`)
// falls back to the `unresolved` default.
function configRecordEvidenceLabels(record) {
  ...
  // package.json: settings/scripts/dependencies/developmentDependencies are all authored
  // declarations (R1) - unverified against node_modules or actual imports. `toolchain` is
  // the exception: it is the tool's own filtered subset of `dependencies`, not something
  // the file declares.
  ...
  // package-lock.json and vercel.json: whole-file JSON parses of declared configuration
  // (R2), so `summary`/`settings` faithfully represent the file under R1.
  if (record.type === "package-lock" || record.type === "vercel") {
    return {
      default: "documented_intent",
      fields: sortedFields({
        path: "observed_fact",
  ...
  // tsconfig: the one case of a real structural JSON/JSONC parse (R2) among this
  // collector's record shapes, so `settings.compilerOptions`/`references`/`entryPoints`
  // faithfully represent the file, unlike everything regex-based below.
  if (record.type === "typescript") {
    return {
      default: "documented_intent",
      fields: sortedFields({
        path: "observed_fact",
  ...
  // .env.example itself: `settings.variableCount` is a count over declarations, which are
  // observed_fact (dotenv is a trivial, fully-parsed line format, per R2) - a count
  // inherits the label of the set it summarizes.
  if (record.type === "environment-example") {
    return {
  ...
  // netlify.toml / supabase/config.toml: `parseTomlOverview` is a line scanner, never a
  // real TOML parser (R2), so `settings.sections`/`.keys`/`environmentVariables` are all
  // mechanical_inference across the board.
  ...
  // Script configs (vite/eslint/tailwind/postcss/vitest and the generic "configuration"
  // fallback): `usesDefineConfig`/`flatConfig` and `references` all come from a regex over
  // unparsed source, never a real parser (R2).
  return {
    default: "mechanical_inference",
    fields: sortedFields({ path: "observed_fact" }),
  };
}

// metadata.environmentVariables[]: `name` and `declarations` are observed_fact (dotenv is a
// trivial, fully-parsed line format). `descriptions` are documented_intent. `references` and
// `accessStyles` are mechanical_inference (regex over JS/TS source, R2). `classification` is
// value-dependent per R3: mechanical_inference normally, unresolved when the value is the
// literal "unknown".
function environmentVariableEvidenceLabels(record) {
  const fields = {
    name: "observed_fact",
  ...
  if (record.classification === "unknown") fields.classification = "unresolved";

  return { default: "mechanical_inference", fields: sortedFields(fields) };
}
...
export function collectConfiguration({ root = process.cwd() } = {}) {
  ...
  for (const directory of SOURCE_DIRECTORIES) {
    if (existsSync(join(repositoryRoot, directory))) {
      scanSourceDirectory(repositoryRoot, directory, environmentVariables, warnings, observations);
    }
  }
  collectRootEnvironmentReferences(repositoryRoot, environmentVariables, warnings, observations);

  const packageConfiguration = records.find((record) => record.type === "package");
  const environment = environmentRecords(environmentVariables, observations);

  return {
    status: warnings.length ? "partial" : "complete",
    records: records
      .sort((left, right) => compareText(left.path, right.path))
      .map((record) => ({ ...record, evidenceLabels: configRecordEvidenceLabels(record) })),
    warnings,
    observations: buildObservations(observations),
    metadata: {
      ...
      environmentVariables: environment.map((record) => ({
        ...record,
        evidenceLabels: environmentVariableEvidenceLabels(record),
      })),
      evidenceLabels: {
        default: "documented_intent",
        fields: sortedFields({ toolchain: "mechanical_inference" }),
      },
    },
  };
}
