...
function workflowPaths(root) {
  ...
  return entries
    // S7: silently skip a private-looking filename (e.g. ".env.yml"), same as every other
    // silent skip in this walker -- nobody asked for this specific file.
    .filter((entry) => /\.(?:yaml|yml)$/.test(entry.name) && !isPrivateEnvironmentFile(entry.name))
