...
import { isPrivateEnvironmentFile } from "./private-files.mjs";
import {
...
function repositoryConfig(root, warnings) {
  ...
  // S7: a repository's own config can name any path it likes here (only safePath's
  // path-escape check gates it), so a declared `.env` would otherwise be read like any
  // other authority document. Refuse it instead -- this is the repository's own
  // configuration asking for something the tool will not honor, which is a warning per
  // owner decision 3.2 in docs/decisions/2026-09-22-schema-v3-structural-guard.md's
  // successor S7 design, not silence and not an observation.
  if (parsed?.authorityDocuments && typeof parsed.authorityDocuments === "object") {
    for (const [rawPath, rawRole] of Object.entries(parsed.authorityDocuments)) {
      ...
      if (!path || typeof rawRole !== "string") continue;
      if (isPrivateEnvironmentFile(path)) {
        warnings.push(
          `Repository intelligence config declared a private environment file as an authority document; refusing to read it: ${path}.`,
        );
        continue;
      }
  ...
  if (Array.isArray(parsed?.requiredAuthorityDocuments)) {
    for (const rawPath of parsed.requiredAuthorityDocuments) {
      ...
      requiredAuthorityDocuments.push(path);
    }
  }
...
function discoveredPlanningPaths(root, config) {
  ...
  function visit(directory) {
    ...
    for (const entry of entries.sort((a, b) => compareText(a.name, b.name))) {
      ...
      // Silent, matching the existing excluded-directory-name behavior: nobody asked for
      // this specific file the way a config's authorityDocuments/requiredAuthorityDocuments
      // entry is a specific ask (see repositoryConfig above, which warns instead).
      else if (
        entry.isFile() &&
        /\.(?:md|mdx|rst|txt)$/i.test(entry.name) &&
        !isPrivateEnvironmentFile(path)
      ) paths.add(path);
    }
  }
...
export function collectPlanning({ root = process.cwd() } = {}) {
  ...
  // S3: a declared-but-missing document is a fact about the repository, not a collection
  // failure -- the tool read everything it could -- so it is an observation, not a
  // warning. See docs/decisions/2026-09-20-warnings-vs-observations.md section 2.4 and
  // section 5 step 4.
  for (const path of config.requiredAuthorityDocuments) {
    if (!planningPaths.includes(path)) {
      observations.push({
        id: "required-authority-document-absent",
        message: `Expected repository authority document was absent: ${path}.`,
      });
    }
  }
