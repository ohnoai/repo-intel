...
// Generic defaults only. Anything repository-specific belongs in that repository's own
// .repo-intelligence.json, so this collector can be pointed at any repo with --root and
// produce clean output instead of warning about documents that were never supposed to
// exist there.
...
// Both task conventions in common use: some repos keep tasks under docs/tasks/, others
// use a bare tasks/ directory. Scanning both by default means neither layout is invisible.
...
function titleFrom(text, path) {
  ...
  return sanitizeMetadataText(heading?.[1]?.trim() || path.split("/").at(-1));
}
...
function documentRecord(path, text, classification) {
  ...
  return {
    ...
    owner: frontMatterValue(text, "owner"),
  };
}
...
function repositoryConfig(root, warnings) {
  ...
  if (!existsSync(configPath)) {
    return { authorityDocuments, planningDirectories, requiredAuthorityDocuments };
  }
...
export function collectPlanning({ root = process.cwd() } = {}) {
  ...
  try { if (!statSync(repositoryRoot).isDirectory()) throw new Error("not a directory"); }
    ...
    return {
      ...
      metadata: { documentCount: 0, authorityDocumentCount: 0, externalContext: { source: "google-drive-keeper-note", status: "unavailable", required: false, accessed: false } },
    };
  }
  ...
  for (const path of planningPaths.slice(0, MAX_DOCUMENTS)) {
    ...
    records.push(documentRecord(path, text, config.authorityDocuments.get(path)));
  }
